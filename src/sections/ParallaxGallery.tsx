...
import { Terminal, ArrowRight, Shield, Crosshair } from 'lucide-react';
...
const ParallaxGallery = () => {
  ...
  const scrollToTour = () => {
    const tourSection = document.getElementById('engagements');
    if (tourSection) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {parallaxGalleryConfig.parallaxImagesTop.map((image) => (
            <div
              key={image.id}
              className="relative flex-shrink-0 w-[400px] h-[250px] overflow-hidden rounded-lg image-hover-scale border border-[#333]"
            >
              <img
                src={image.src}
                alt={image.alt}
              ...
              <div className="absolute inset-0 border border-[#e63946]/0 hover:border-[#e63946]/30 transition-colors duration-300" />
            </div>
          ))}
        </div>
      ...
      {/* Marquee Section */}
      <div className="relative py-8 bg-[#0a0a0a] overflow-hidden border-y border-[#e63946]/20">
        <div className="animate-marquee flex whitespace-nowrap">
          {[...Array(8)].map((_, i) => (
            <span
              key={i}
              className="flex items-center gap-8 mx-8 text-2xl font-display text-[#e63946]/30"
            >
      ...
      >
        ...
        >
          {parallaxGalleryConfig.galleryImages.map((image, index) => (
            ...
            >
              <div className="relative w-[450px] h-[300px] overflow-hidden rounded-xl border border-[#333] group-hover:border-[#e63946]/50 transition-colors duration-300">
                <img
                  src={image.src}
                  alt={image.title}
              ...
              {/* Index number */}
              <div className="absolute -top-8 -left-4 font-mono-custom text-7xl text-[#e63946]/10 font-bold">
          ...
          <div className="flex-shrink-0 flex flex-col items-center justify-center w-[300px] h-[300px]">
            <button
              ...
              className="group flex flex-col items-center gap-4 text-white hover:text-[#e63946] transition-colors"
            >
              <div className="w-20 h-20 rounded-full border border-white/20 group-hover:border-[#e63946] flex items-center justify-center transition-colors">
        ...
        <div className="absolute bottom-12 left-12 right-12 h-px bg-white/10">
          <div className="h-full bg-[#e63946]/50 w-0" id="gallery-progress" />
        </div>
      </div>
    </section>
  );

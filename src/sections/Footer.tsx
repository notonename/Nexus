...
import { Instagram, Twitter, Youtube, Github, Mail, Key, MapPin, ExternalLink, Shield } from 'lucide-react';
      ...
      <div className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        >
          <div className="relative w-full max-w-2xl aspect-[2/3] mx-auto">
            ...
            <div className="absolute inset-0 bg-gradient-to-b from-[#050505] via-transparent to-transparent opacity-50" />
          </div>
        </div>
        ...
        >
          ...
          <p className="font-mono-custom text-lg text-[#e63946]/60 uppercase tracking-[0.5em] mt-4">
        ...
        <div className="absolute bottom-20 left-12 z-20">
          ...
          <p className="font-mono-custom text-sm text-[#e63946]/60">{footerConfig.artistSubtitle}</p>
        </div>
      </div>

      {/* Footer content */}
      <div className="relative bg-[#050505] py-20 px-6 md:px-12">
        {/* Top divider */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-20">
            ...
            <div>
              <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-10 rounded bg-[#e63946]/20 flex items-center justify-center border border-[#e63946]/40">
                ...
                {footerConfig.socialLinks.map((social) => {
                  ...
                  return (
                    <a
                      key={social.label}
                      href={social.href}
                      className="w-10 h-10 rounded border border-white/10 flex items-center justify-center text-white/60 hover:text-[#e63946] hover:border-[#e63946]/50 transition-colors"
              ...
              <ul className="space-y-3">
                {footerConfig.quickLinks.map((link) => (
                  <li key={link}>
                    <a
                      href="#"
                      className="text-sm text-white/50 hover:text-[#e63946] transition-colors flex items-center gap-2 group"
                    >
                      <span>{link}</span>
            ...
            <div>
              ...
              <ul className="space-y-4">
                <li className="flex items-start gap-3">
                  <Mail className="w-4 h-4 text-[#e63946]/60 mt-0.5" />
                  <div>
                    <p className="text-sm text-white/50">{footerConfig.emailLabel}</p>
                    <a href={`mailto:${footerConfig.email}`} className="text-sm text-white hover:text-[#e63946] transition-colors">
                      {footerConfig.email}
                    </a>
                  </div>
                </li>
                <li className="flex items-start gap-3">
                  <Key className="w-4 h-4 text-[#e63946]/60 mt-0.5" />
                  <div>
                    <p className="text-sm text-white/50">{footerConfig.phoneLabel}</p>
                    <span className="text-sm text-white font-mono-custom">{footerConfig.phone}</span>
                  </div>
                </li>
                <li className="flex items-start gap-3">
                  <MapPin className="w-4 h-4 text-[#e63946]/60 mt-0.5" />
                  <div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-grow px-4 py-3 bg-white/5 border border-white/10 rounded text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-[#e63946]/50"
                />
                <button
                  onClick={handleContactClick}
                  className="px-4 py-3 bg-[#e63946]/20 text-[#e63946] rounded text-sm font-medium hover:bg-[#e63946]/30 transition-colors"
                >
            ...
            <div className="mb-12">
              ...
              <div className="grid grid-cols-4 md:grid-cols-8 gap-2">
                {footerConfig.galleryImages.map((image, index) => (
                  <div
                    key={image.id}
                    className="relative aspect-square overflow-hidden rounded footer-grid-item cursor-pointer border border-[#333] hover:border-[#e63946]/50 transition-colors"
          ...
          <div className="pt-8 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex gap-6">
              {footerConfig.bottomLinks.map((link) => (
                <a key={link} href="#" className="text-xs text-white/30 hover:text-[#e63946]/60 transition-colors">
                  {link}
                </a>
              ))}
            </div>
